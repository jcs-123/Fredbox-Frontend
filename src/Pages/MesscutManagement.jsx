...
const MesscutManagement = () => {
  ...
  const fetchMesscuts = async () => {
    ...
    setError("");
    try {
      const res = await axios.get(`${API_URL}/api/messcut/all-details`);
      setMesscuts(res.data.data || []);
    } catch {
      setError("Failed to load messcut data");
    } finally {
      setLoading(false);
    }
  };
 ...
 const openDateModal = (r) => {
  ...
  const exportExcel = async () => {
    ...
    const ws = wb.addWorksheet("Messcut");

    const rows = filtered.map((r, i) => ({
      "#": i + 1,
      Name: r.name,
      Admission: r.admissionNumber,
      Leave: formatDate(r.leavingDate),
      Return: formatDate(r.returningDate),
      Days: calculateMesscutDays(r.leavingDate, r.returningDate),
      Status: r.status,
    }));
    ...
    saveAs(new Blob([buf]), "Messcut_Report.xlsx");
  };
  ...
  const exportPDF = () => {
    ...
    doc.save("Messcut_Report.pdf");
  };

  /* ================= UI ================= */
  return (
    <Container fluid className="py-4 bg-light">
      <h3 className="fw-bold text-primary mb-3">Messcut Management</h3>
        ...
        <Table bordered hover size="sm">
          ...
          <tbody>
            {loading ? (
              ...
              filtered.map((r, i) => (
                <tr key={r._id || `${r.admissionNumber}-${i}`}>
                  <td>{i + 1}</td>
                  <td>{r.name}</td>
      ...
      <Modal show={showDateModal} onHide={() => setShowDateModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Change Messcut Dates</Modal.Title>
        </Modal.Header>
          ...
          <Modal.Body>
            <Form.Group>
              ...
              <Form.Control
                ...
                onChange={(e) =>
                  setDateForm({ ...dateForm, newLeavingDate: e.target.value })
                }
                required
              />
            </Form.Group>
            ...
            <Form.Group className="mt-2">
              ...
              <Form.Control
                ...
                onChange={(e) =>
                  setDateForm({ ...dateForm, newReturningDate: e.target.value })
                }
